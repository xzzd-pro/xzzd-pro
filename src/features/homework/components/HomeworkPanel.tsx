...
function hasValidDate(date: Date): boolean {
...
function processHomeworks(
  ...
  const processed = homeworks.map((hw) => ({
    ...
    ...(() => {
      const startAt = new Date(hw.start_time)
      const deadline = new Date(hw.end_time)
      const now = Date.now()
      const isNotStarted = hasValidDate(startAt) && startAt.getTime() > now
      const isDeadlinePassed =
        hasValidDate(deadline) && deadline.getTime() <= now
      const isClosed = !isNotStarted && (hw.is_closed || isDeadlinePassed)

      return {
        isClosed,
        isNotStarted,
        canSubmit: !isNotStarted && !isClosed,
...
function getDeadlineInfo(
  ...
  if (isNotStarted && hasValidDate(startAt)) {
    return {
      variant: "soon",
      text: `未开始：${formatAbsoluteDeadline(startAt)} 开始`
    }
  }
  ...
  return (
    <Accordion type="multiple" className="w-full space-y-3">
      {homeworks.map((homework) => {
        ...
        const statusText = homework.isNotStarted
          ? "\u672a\u5f00\u59cb"
          : homework.isClosed
            ? "\u5df2\u7ed3\u675f"
            : "\u8fdb\u884c\u4e2d"
                    ...
                    <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                      <Badge
                        ...
                        className={cn(
                          ...
                          homework.canSubmit && "border-primary/20 bg-primary/10 text-primary",
                          homework.isNotStarted &&
