...
import type { HomeworkSubmission, ProcessedHomework } from "@/types"
...
export function HomeworkItem({ homework, userId }: HomeworkItemProps) {
  const [submissions, setSubmissions] =
    React.useState<HomeworkSubmission[] | null>(null)
  ...
  React.useEffect(() => {
    if (!isExpanded || submissions !== null) return
  ...
  }, [homework.id, isExpanded, submissions, userId])

  const latestSubmission = submissions?.find(
    (submission) =>
      submission.is_latest_version && submission.marked_submitted
  )
  ...
  const formatDeadline = (
    ...
    if (isNotStarted && Number.isFinite(startAt.getTime())) {
      return `将于 ${startAt.toLocaleString("zh-CN")} 开始`
    }
  ...
  return (
    <AccordionItem
      ...
      className={cn(
        ...
        homework.isClosed
          ? "border-l-muted-foreground opacity-85"
          : homework.isNotStarted
            ? "border-l-yellow-500"
            : "border-l-primary"
      )}
    >
          ...
          <div className="flex justify-between items-start gap-4 mb-4">
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              <Badge
                ...
                className={cn(
                  !isSubmitted && "bg-yellow-500 text-gray-900 border-transparent"
                )}
              >
                {isSubmitted ? "已提交" : "未提交"}
              </Badge>
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn(
                  ...
                  homework.canSubmit && "text-destructive animate-pulse"
                )}
              >
                {formatDeadline(
                  homework.deadline,
      ...
      <AccordionContent className="px-6 pb-6 pl-[60px]">
        <div className="space-y-6">
          <div>
            <h4 className="text-[15px] font-semibold mb-4 pb-2 border-b border-border">
              已提交文件
            </h4>
            ...
            {submissionsLoading ? (
              ...
              <>
                <p className="text-sm text-muted-foreground mb-3">
                  提交时间：
                  {new Date(latestSubmission.submitted_at).toLocaleString("zh-CN")}
                </p>
